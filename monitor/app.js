...
const agentMethods = {
  ...
  tool_search: "查找可用工具",
  web_search: "搜索网页",
  "functions.exec": "执行工具脚本",
  "functions.wait": "等待工具返回",
  Read: "读取文件",
  Bash: "执行命令",
  Edit: "编辑文件",
  Write: "写入文件",
...
function tone(status) {
  return status === "delivered"
    ? "success"
    : ["delivery_failed", "interrupted"].includes(status)
      ? "error"
      : status === "running"
        ? "running"
        : "waiting";
...
function renderList() {
  ...
  for (const run of runs) {
    const button = el(
      "button",
      `run-card${run.id === selected ? " selected" : ""}`,
    );
    button.type = "button";
    button.setAttribute("aria-current", run.id === selected ? "true" : "false");
    button.append(el("div", "run-card-title", run.question));
    const meta = el("div", "run-card-meta");
    meta.append(
      el("span", "", time(run.startedAt)),
      el("span", `badge ${tone(run.status)}`, states[run.status] || run.status),
    );
    button.append(meta);
    button.addEventListener("click", () => {
      selected = run.id;
      follow = run.id === runs[0]?.id;
      render();
    });
    $("runs").append(button);
  }
...
function toolCounts(run, group) {
  ...
  for (const step of run.steps)
    if (step.kind === (group === "browser" ? "command" : "agent"))
      names.set(step.title, (names.get(step.title) || 0) + 1);
  return [...names].map(([name, count]) => ({ name, count }));
}

function renderTools(run, group) {
  const counts = [...toolCounts(run, group)].sort(
    (a, b) => b.count - a.count || a.name.localeCompare(b.name),
  );
  const total = counts.reduce((sum, item) => sum + item.count, 0);
  $(`${group}-total`).textContent = `${total} 次 · ${counts.length} 种`;
  const box = $(`${group}-tools`);
  box.replaceChildren();
  if (!counts.length) {
    box.append(
      el(
  ...
  table.setAttribute(
    "aria-label",
    group === "browser" ? "浏览器工具调用分布" : "Agent 工具调用分布",
  );
  const head = el("thead"),
    tr = el("tr");
  for (const name of ["工具", "占本类调用", "次数"]) {
    const th = el("th", "", name);
    th.scope = "col";
    tr.append(th);
  }
  ...
  for (const item of counts) {
    const row = el("tr"),
      label = el("td"),
      bar = el("td"),
      count = el("td", "tool-count", item.count);
...
function render() {
  if (follow || !runs.some((run) => run.id === selected))
  ...
  $("errors").textContent = run.steps.filter(
  ...
  $("error-note").textContent =
    `当前保留步骤中：浏览器失败 ${browserFailures} 条 · Agent 工具失败 ${agentFailures} 条 · 消息与状态失败 ${failures.length - browserFailures - agentFailures} 条。同一次浏览器失败可能同时产生 Agent 命令失败，两层未去重，合计不代表独立故障数。`;
  $("follow").setAttribute("aria-pressed", String(follow));
  let note =
    run.status === "delivered"
      ? "插件已确认保存最终回复，本轮已结束。任务是否达成请查看回复内容。"
      : "等待期间会持续计时；工具返回后自动显示结果。";
  if (run.messages > 1)
    note += ` 本轮包含 ${run.messages} 条连续提问；当前协议无法逐条关联工具调用，因此合并展示。`;
  if (run.omittedSteps) note += ` 更早的 ${run.omittedSteps} 个步骤已省略。`;
  if (run.agentMixed)
    note +=
      " Agent 同一轮混入了其他来源的消息；无法明确归属的后续调用未分配到此问题。";
  ...
  for (const [index, step] of [...run.steps]
    .sort((a, b) => a.startedAt - b.startedAt)
    .entries()) {
    const category = ["command", "agent"].includes(step.kind)
      ? step.kind
      : "message";
    ...
    details.open = expanded.has(step.id)
      ? expanded.get(step.id)
      : ["question", "final"].includes(step.kind);
    details.addEventListener("toggle", () => {
      if (details.isConnected) expanded.set(step.id, details.open);
    });
    ...
    summary.append(
      el(
        "span",
        "step-number",
        String(index + 1 + run.omittedSteps).padStart(2, "0"),
      ),
      el(
        "span",
        `source-label ${category}`,
        { command: "浏览器", agent: "Agent", message: "消息" }[category],
      ),
      el("span", "step-title", title),
    );
    const timing = el(
      "span",
      "step-timing",
    ...
    summary.append(
      ...
      el(
        "span",
        `badge ${step.status}`,
        stepStates[step.status] || step.status,
      ),
    );
    ...
    content.append(
      el(
        "p",
        "muted",
        `开始于 ${time(step.startedAt, true)}${step.endedAt ? ` · 返回于 ${time(step.endedAt, true)}` : ""}`,
      ),
    );
    ...
    if (step.invocation) {
      let args;
      try {
        args = JSON.parse(step.invocation.json);
      } catch {
        /* older diagnostic format */
      }
      if (typeof args?.cmd === "string")
        addDetail(content, "执行命令 · cmd", args.cmd);
      addDetail(
        content,
        step.kind === "agent-input"
          ? "Agent 收到的消息文本"
          : "传给执行工具的参数 · JSON",
        typeof args === "string" ? args : step.invocation.json,
      );
      const notes = [];
      if (step.invocation.redacted) notes.push("识别到的凭证或图片编码已隐藏");
      if (step.invocation.truncated)
        notes.push(
          `参数过长，部分内容已截断（原始 ${step.invocation.originalBytes} 字节）`,
        );
      if (step.kind === "agent") notes.push("这些参数由 Agent 发给执行工具");
      if (notes.length)
        content.append(el("p", "muted", notes.join("；") + "。"));
    } else if (
      step.kind === "agent" &&
      /(?:^|\.)exec_command$/.test(step.title)
    ) {
      content.append(
        el(
          "p",
          "muted",
          "此旧记录只有摘要；若当前扫描的日志中能找到对应调用，会自动补充参数。",
        ),
      );
    }
  ...
  if (!timeline.children.length)
    timeline.append(el("li", "tool-empty", "此类型暂无步骤。"));
  tick();
}
...
function tick() {
  ...
  if (connected)
    for (const node of document.querySelectorAll("[data-started-at]"))
      node.textContent = duration(Date.now() - Number(node.dataset.startedAt));
}

async function refresh() {
  try {
    const response = await fetch(
      `/monitor/api?revision=${encodeURIComponent(revision)}`,
    ...
    $("connection").textContent = count
      ? `${count} 个浏览器已连接`
      : "服务在线 · 等待浏览器";
    $("connection").classList.toggle("offline", !count);
    const connections = $("connection-details");
    ...
    $("agent-source").textContent =
      data.agentSource?.message || "Agent 工具采集未启用";
    $("agent-source").classList.toggle(
      "source-warning",
      data.agentSource?.status !== "connected",
    );
    $("warning").hidden = !data.storageError;
    $("warning").textContent = data.storageError || "";
    if (!data.unchanged) {
      runs = data.runs;
    ...
    connected = false;
    $("connection").textContent = "监控连接中断";
    $("connection").classList.add("offline");
    $("updated").textContent = "正在自动重连";
    $("warning").hidden = false;
    $("warning").textContent =
      "暂时无法连接本地 Relay。页面保留最后一次记录，恢复连接后自动更新。";
  } finally {
