...
test("opaque decision contract travels through C4, attachments stay on Agent host, and responses use a correlated CLI", async (t) => {
  ...
  assert.ok(
    (
      await waitFor(() => frames.find((frame) => frame.type === "ready"))
    ).capabilities.includes("agent-loop-v1"),
  );
  ...
  assert.equal(
    (
      await waitFor(() =>
        frames.find((frame) => frame.requestId === request.id),
      )
    ).state,
    "queued",
  );
  ...
  assert.ok(
    content.startsWith(
      `[Browser] [Extension decision request ${relay.ext.connectedIds()[0]}/request-1`,
    ),
  );
  assert.ok(content.includes("scripts/decision.js"));
  assert.ok(content.includes("c4-send.js browser-remote"));
  assert.ok(content.includes("|req:request-1|status:done"));
  assert.ok(
    args.includes("--no-reply"),
    "C4 must not append a reply route tied to the first round instead of the current request",
  );
  ...
  const oldReply = await fetch(base + "/chat", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ endpoint, text: "old route" }),
  });
  assert.equal(oldReply.status, 404);
  const decision = {
    kind: "a-future-extension-format",
    payload: { anything: [1, 2] },
  };
  const child = spawn(
    process.execPath,
    [path.resolve(__dirname, "../scripts/decision.js"), endpoint, request.id],
    {
      env: { ...process.env, BROWSER_REMOTE_AGENT_URL: base },
  ...
  assert.ok(
    JSON.parse(stdout).next.replyCommands.blocked.includes(
      "|req:request-2|status:blocked",
    ),
  );
  assert.equal(
  ...
  assert.equal(
    ...
    1,
    "duplicate request must not enqueue again",
  );
  ws.send(
    JSON.stringify({
      type: "agent-turn-end",
