 ...
 */
function deliverChatToC4({ keyId, text }, logFn = log) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        logFn(`chat: delivered to C4 (endpoint ${keyId}, ${text.length} chars)`);
...
function start({ extPort = EXT_PORT, agentPort = AGENT_PORT, onChat = deliverChatToC4 } = {}) {
  ...
  ext.on('chat', (msg) => {
    Promise.resolve(onChat(msg)).catch((err) => log(`chat: unexpected delivery error: ${err.message}`));
  });
