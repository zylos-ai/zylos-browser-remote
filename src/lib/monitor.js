...
function summarize(value, depth = 0, input = false) {
  ...
  if (typeof value === "string")
    return value.length > 1000 ? value.slice(0, 1000) + "… [已截断]" : value;
  if (value === null || typeof value !== "object") return value;
  if (Array.isArray(value))
    return value.slice(0, 12).map((item) => summarize(item, depth + 1, input));
...
class Monitor {
  constructor({ file, now = Date.now } = {}) {
    ...
    this.agentSource = {
      status: "disabled",
      message: "Agent 工具采集未启用；当前仅记录浏览器调用",
    };
    if (file) {
      try {
        if (fs.statSync(file).size > MAX_FILE_BYTES)
          throw new Error("监控记录超过大小限制");
        const saved = JSON.parse(fs.readFileSync(file, "utf8"));
        if (saved.version !== 1 || !Array.isArray(saved.runs))
          throw new Error("监控记录格式无效");
        this.runs = saved.runs
          .filter(
            (run) =>
              run && typeof run.id === "string" && Array.isArray(run.steps),
          )
          .slice(-MAX_RUNS);
        ...
        if (error.code !== "ENOENT")
          this.storageError = "历史记录读取失败；本次仅保存在内存中";
      }
    }
  }
  ...
  agentStarted(
    run,
    { name, callId, sessionId, turnId, params, invocation, at },
  ) {
    this.countTool(run, "agent", name);
    const step = this.add(run, "agent", name, {
      status: "running",
      callId,
      sessionId,
      turnId,
  ...
  received({ keyId, label, text, chatId, context }) {
    ...
    run.messages++;
    this.add(run, "question", run.messages > 1 ? "收到追加提问" : "收到提问", {
      text: text.slice(0, 8000),
      chatId,
      ...(context === undefined ? {} : { context: context.slice(0, 16000) }),
    });
    const step = this.add(run, "queue", "投递到 Agent 队列", {
      status: "running",
    });
    run.status = run.steps.some(
      (item) => item.kind === "command" && item.status === "running",
    )
      ? "running"
      : "queuing";
    return { run, step };
  ...
  intake(ticket, result) {
    ...
    step.status = result?.ok
      ? "success"
      : ["C4_DELIVERY_TIMEOUT", "C4_DELIVERY_UNCONFIRMED"].includes(
            result?.code,
          )
        ? "unknown"
        : "error";
    step.result = result?.ok ? "队列已接收，等待 Agent 决策" : undefined;
    step.error = result?.code;
    if (["queuing", "waiting"].includes(run.status)) {
      run.status =
        result?.ok || run.messages > 1
          ? "waiting"
          : step.status === "unknown"
            ? "uncertain"
            : "delivery_failed";
  ...
  extensionEnded({ keyId, status, text }) {
    ...
    this.add(
      run,
      "final",
      status === "done" ? "插件已保存最终回复，本轮结束" : "插件结束本轮任务",
  ...
  actionStarted(keyId, { method, params, requestId }) {
    ...
    const step = this.add(run, "command", method, {
      status: "running",
      requestId,
      params: detail(params || {}, true),
    });
    run.status = "running";
    return { run, step };
  }

  actionEnded(ticket, result, error) {
    if (!ticket) return;
    const { run, step } = ticket;
    step.endedAt = this.now();
    step.durationMs = step.endedAt - step.startedAt;
  ...
  connection(keyId, connected) {
    ...
    if (run)
      this.add(
        run,
        "connection",
        connected ? "浏览器已连接" : "浏览器连接断开",
        { status: connected ? "success" : "unknown" },
      );
    this.changed();
  }
